...
| Administration and redrive | Strong-authenticated, reasoned, time-bounded, scoped, audited export/quarantine/redrive operations | Exact Human identity, sealed scope, strong-authentication port, and one-use grant lifecycle are implemented only as process-local reference contracts; adapter, durable enforcement, action execution, and exercises belong to issues #30, #31, #34, and #64 |
...
| Audit, telemetry, and archive | Define bounded canonical events, sequence and hash-chain verification, fixed retention decisions, signed-export verification inputs, and process-local one-use privileged grants; durable production, archive, signer, and runtime enforcement remain deferred | `docs/architecture/0011-tamper-evident-audit-and-privileged-administration.md:36-310` |
...
| A-05 | Audit events, integrity anchors, manifests, exports, and recovery evidence | Completeness relative to an independently trusted terminal checkpoint, ordering, tamper evidence, attribution, confidentiality, and retained availability | `docs/architecture/0011-tamper-evident-audit-and-privileged-administration.md:63-254` |
...
| TB-09 | Operator/admin to export, quarantine, redrive, recovery, policy, migration, and break-glass capabilities | Platform identity separate from Workspace policy, future strong-authentication verifier adapter, exact action/sealed scope/reason, one-use grant of at most 15 minutes, immutable audit, and durable cross-node enforcement before execution | OWN-OPERATIONS |
...
| High | **TM-012 — Break-glass, redrive, migration, or recovery authority is abused.** ACT-PLATFORM bypasses ordinary policy without durable accountability. | Broad console/database access, reusable admin credential, missing reason/expiry, or direct edits | Multi-workspace data/provider mutation and audit bypass | Separate exact Human platform-administrator identities, sealed targets, a strong-authentication verifier port, bounded one-use grants, and audit elevation projections are implemented as process-local reference boundaries; no verifier adapter, durable compare-and-set, action execution, or runtime audit exists | Implement the strong-authentication adapter and durable cross-node grant compare-and-set; preserve separate role, exact action/scope/reason, 15-minute ceiling, one-use consumption, immutable audit, and no routine direct edits | Follow-up: Issues #24, #30, #31, #34, and #64; verification: Issues #27 and #64 |
