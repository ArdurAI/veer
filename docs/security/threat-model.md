...
| --- | --- | --- | --- |
| TB-01 | External clients and GitOps automation to API: token, request, desired state, source revision, and idempotency key | The reference harness enforces literal loopback binding, body/time limits, one stripped header-only Bearer value, a fixed credential, a closed action gate, correlation, and absent-credential rejection; TLS, configured OIDC, authoritative policy, and public destination controls remain production requirements | OWN-IDENTITY |
...
- The accepted authorization reference defaults to denial, matches private
  ...
  hierarchy IDs. Its domain and OpenAPI tests do not establish that either gate
  is wired into a production API or worker.
...
- The provider-neutral principal, strict Bearer extraction, configured OIDC JWT
  ...
  server is a loopback harness with one fixed credential and closed action gate;
  it does not invoke OIDC or PolicySet evaluation. Production API/worker
  authorization wiring, store schema, durable
