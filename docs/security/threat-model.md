...
| DC-AUDIT | Audit | Actor and authentication method action target policy decision request operation provider attempt outcome timestamp source integrity and elevation reason | Encrypted append-only relational records with independently protected integrity anchors and immutable encrypted object archive; secret values and raw request or response bodies are prohibited | TLS for query export replication and recovery; export includes verifiable sequence integrity manifest and scope but no credential material | Workspace-scoped authorized readers; separate security or platform operator access requires strong authenticated scoped audited elevation; runtime can append but not mutate or delete | Security events belong here rather than only in platform logs; ordinary logs may carry an opaque event or operation ID but not duplicate sensitive audit payload | 90 days queryable and 365 days immutable encrypted archive; shorter retention requires reviewed security decision and longer retention requires security privacy storage and cost review | Expire only through reviewed lifecycle after retention and legal or incident holds; deletion or failed export is security-relevant and audited; integrity metadata proves gaps | OWN-DATA | Issues #27 and #64 |
